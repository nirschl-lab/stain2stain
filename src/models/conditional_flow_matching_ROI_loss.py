...
class ConditionalFlowMatchingLitModule(LightningModule):
    ...
    def model_step(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor]) -> torch.Tensor:
        ...
        # ---- ROI Charbonnier reconstruction term (paired supervision) ----
        # prediction in "IHC image space" to compare to x1.
        # using interpolated point xt as the current "generated" sample.
        ihc_pred = xt
        ihc_true = x1
        ...
        diff = ihc_pred - ihc_true                          # (B,C,H,W)
        charb = torch.sqrt(diff * diff + eps_charb * eps_charb)

        C = ihc_pred.shape[1]
        roi_charb = (charb * m).sum() / (m.sum() * C + eps_area)

        lambda_roi = getattr(self, "lambda_roi", 1.0)        # or set self.lambda_roi in __init__
        loss = loss_fm + lambda_roi * roi_charb
    ...
    def generate(
        ...
        node = NeuralODE(
            self.net,  # Use wrapper that returns only velocity field
