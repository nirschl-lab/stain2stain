...
class ConditionalFlowMatchingLitModule(LightningModule):
    ...
    def model_step(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], use_mask_toggle: bool = False) -> torch.Tensor:
        """Perform a single model step for training or validation.

        :param batch: (source_img, target_img, target_seg_mask)
        :param use_mask_toggle: If True, randomly zero out mask 50% of the time
        :return: loss
        """
        source_img, target_img, mask = batch          # mask: (B,1,H,W), ROI=1
        x0, x1 = source_img, target_img

        # Toggle mask: 50% probability to zero it out during training
        if use_mask_toggle and torch.rand(1).item() < 0.5:
    ...
    def training_step(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        """
        # Enable mask toggling during training (50% probability)
        loss = self.model_step(batch, use_mask_toggle=True)
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        :param batch_idx: The index of the batch
        """
        # Zero out mask during validation
        source_img, target_img, mask = batch
        mask = torch.zeros_like(mask)
        batch = (source_img, target_img, mask)
        
        loss = self.model_step(batch, use_mask_toggle=False)
    ...
    def generate(
        ...
        # Zero out mask during generation
        mask = torch.zeros_like(mask)
